...
export default function Player({ src, poster }: Props) {
  ...
  useEffect(() => {
    ...
    const getHttpStatus = (data: ErrorData): number | undefined => {
      return (data?.response?.code as number) ?? (data?.networkDetails?.status as number);
    };
    ...
    if (video.canPlayType('application/vnd.apple.mpegurl')) {
      ...
      const cfg: Record<string, boolean | object | Record<string, number | string>> = {
        enableWorker: true,
        debug: false,
      };
      try {
        cfg.fragLoadPolicy = {
          default: { retry: { maxNumRetry: 1, retryDelayMs: 1500, maxRetryDelayMs: 4000 } },
        };
        cfg.manifestLoadPolicy = {
          default: { retry: { maxNumRetry: 1, retryDelayMs: 1500, maxRetryDelayMs: 4000 } },
        };
        cfg.playlistLoadPolicy = {
          default: { retry: { maxNumRetry: 1, retryDelayMs: 1500, maxRetryDelayMs: 4000 } },
        };
      } catch {}
