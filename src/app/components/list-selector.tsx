...
export default function ListSelector({ onPlayListSelected }: Props) {
  ...
  const onCreatePlaylist = async (listName: string, m3uUrl: string) => {
    ...
    if (!name) {
      setError('Playlist name is missing');
      return;
    }
    if (!url) {
      setError('Playlist url is missing');
      return;
    }
  ...
  return (
    <Stack spacing={3} maxWidth={'800px'} width={'100%'}>
      <Stack content={'center'} alignItems={'center'}>
        ...
        <Typography variant={'h3'} fontWeight={700} color={'#00ffa3'}>
          DUNIPTV
        </Typography>
      </Stack>
      <Card>
