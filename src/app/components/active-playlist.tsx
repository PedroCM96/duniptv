...
export default function ActivePlaylist({selectedPlaylist, onChangePlaylist}: Props) {
    ...
    const groupChannels: Array<Channel> = selectedGroup ? channels.filter((c) => c.group?.toLowerCase() === selectedGroup.toLowerCase()) : [];


    const showGroups = isMobile ? selectedGroup === null : true;
    const showChannels = isMobile ? selectedChannel === null : !!selectedGroup;
    const showPlayer = isMobile ? selectedGroup !== null && selectedChannel !== null : !!selectedChannel;
    ...
    return <Stack spacing={3} width={'100%'} boxSizing={'border-box'}>
        <Stack alignItems={'center'} flexDirection={'row'} gap={1} justifyContent={'space-between'}>
            <Stack content={'center'} alignItems={'center'} flexDirection={'row'} gap={1} justifyContent={'center'}>
                ...
                <Typography fontWeight={700} color={"primary"}>DUNIPTV</Typography>
            </Stack>
            <GitHubIcon color={"primary"} fontSize={'large'} onClick={() => window.open('https://github.com/PedroCM96/duniptv')} sx={{
                cursor: "pointer",
        ...
        {isMobile && <>
            {showGroups && <GroupList groups={groups} selectedPlaylist={selectedPlaylist} onChangePlaylist={onChangePlaylistClicked} onGroupSelected={onGroupSelected}/>}
            {showChannels && !!selectedGroup && <ChannelList onBackClick={onChannelsBackClick} channels={groupChannels} onChannelLoaded={onChannelLoaded} selectedGroupName={selectedGroup}/>}
            {showPlayer && !!selectedChannel && <PlayerCard playerRef={playerRef} selectedChannel={selectedChannel} onBackClick={onBackClick}/>}
        </>}
        {!isMobile && <>
        <Stack flexDirection={"row"} boxSizing={'border-box'} width={'100%'} flex={1} gap={1}>
            <AnimatedColumn visible={showGroups} targetWidth={'30%'}>
                <GroupList groups={groups} selectedPlaylist={selectedPlaylist} onChangePlaylist={onChangePlaylistClicked} onGroupSelected={onGroupSelected}/>
            </AnimatedColumn>
            <AnimatedColumn visible={showChannels && !! selectedGroup} targetWidth={'30%'}>
                <ChannelList onBackClick={onChannelsBackClick} channels={groupChannels} onChannelLoaded={onChannelLoaded} selectedGroupName={selectedGroup ?? ''}/>
            </AnimatedColumn>
            <AnimatedColumn visible={showPlayer && !!selectedChannel} targetWidth={'40%'}>
                <PlayerCard playerRef={playerRef} selectedChannel={selectedChannel ?? {name: '', url: '', group: '', logo: ''}} onBackClick={onBackClick}/>
            </AnimatedColumn>
            {(!showChannels || !showPlayer) && (
                <Stack direction="column" flex={1} width="100%">
                    <Card sx={{ display: 'flex', flex: 1, justifyContent: 'center', padding: '40px' }} >
                        <Typography variant="subtitle1" noWrap>{'Select a group, then a channel and start watching.'}</Typography>
                    </Card>
                </Stack>
            )}
        </Stack>
