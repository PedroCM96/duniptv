...
export default function Home() {
    ...
    return (
        <Stack flexDirection={'row'} paddingX={3} paddingY={4} boxSizing={'border-box'} justifyContent={'center'}>
            <Stack maxWidth={'1920px'} width={'100%'} alignItems={'center'}>
                {!selectedList && (
                    <ListSelector onPlayListSelected={(pl) => {
                        setSelectedList(pl);
                    }}/>
                )}

                {selectedList && <ActivePlaylist selectedPlaylist={selectedList} onChangePlaylist={onChangePlaylist}/>}
            </Stack>
        </Stack>
    );
}
